...
def ep_compile_batch(episode_ids: List[str], use_llm: bool = False) -> Optional[Dict]:
    ...
    use_llm=True 时调用 LLM 做跨 Episode 归纳（V1）。
    """
    ...
    if use_llm and len(episodes) > 1:
        # V1: LLM 跨 Episode 归纳
        try:
            from core.experience_engine import _llm_batch_compile
            compiled = _llm_batch_compile(episodes)
            if compiled:
                logger.info(f"[COMPILER] LLM 批处理编译成功: {compiled.get('name', '?')}")
                return compiled
        except Exception as e:
            logger.warning(f"[COMPILER] LLM 编译失败，回退 V0: {e}")
